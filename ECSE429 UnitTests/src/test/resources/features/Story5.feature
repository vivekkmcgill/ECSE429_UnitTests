Feature: Marking tasks of a project as done or not done
    ...
    Scenario Outline: A TODO associated to project is marked as done [Normal]
        When a project manager changes the <doneStatus> for a task named <title> to true to mark it as done
        Then the task named <title> has its <doneStatus> field updated to true, and associations remain the same
        Examples:
            | title                  | doneStatus | description                           | tasksof     |
    ...
    Scenario Outline: A TODO associated to project is marked as done [Alternate]
        When a project manager changes the <doneStatus> for a task named <title> to false to re-open it
        Then the task named <title> has its <doneStatus> field updated to false, and associations remain the same
        Examples:
            | title                  | doneStatus | description                           | tasksof     |
            | Add submit button      | true       | Add a big green button to submit form |    Form     |
            | Design search bar      | true       | Design the search bar                 |    Search   |
